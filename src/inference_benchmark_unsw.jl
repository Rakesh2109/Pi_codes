#!/usr/bin/env julia
#=
Inference benchmark — UNSW-NB15 (copy of inference_benchmark.jl, different /tmp prefix).
...
X_test_bin = readlines("/tmp/UNSW_X_test.txt")
Y_test     = readlines("/tmp/UNSW_Y_test.txt")
...
X_train_lines = readlines("/tmp/UNSW_X_train.txt")
x_train = [TMInput([parse(Bool, x) for x in split(l, " ")]) for l in X_train_lines]
y_train = [parse(Int8, l) for l in readlines("/tmp/UNSW_Y_train.txt")]
...
save(compiled_tm, "/tmp/unsw_compiled")
model_size_bytes = filesize("/tmp/unsw_compiled.tm")
model_size_kb = model_size_bytes / 1024

println("\nModel compiled. Size: $(round(model_size_kb, digits=1)) KB")

println("\n", "="^60)
println("  INFERENCE BENCHMARK — UNSW-NB15 (single-threaded)")
println("="^60)
...
raw_path  = "/tmp/UNSW_X_test_raw.bin"
bin_path  = "/tmp/UNSW_binarizer.tsv"
open(raw_path) do io
    global X_raw
...
open("/tmp/inference_benchmark_unsw.txt", "w") do f
    println(f, "="^60)
    println(f, "  INFERENCE BENCHMARK — UNSW-NB15 (single-threaded)")
    println(f, "="^60)
...
println("\nSaved: /tmp/inference_benchmark_unsw.txt")
...
summary_path = joinpath(@__DIR__, "results", "unsw_ml_baselines", "summary.tsv")
