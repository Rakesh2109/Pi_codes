...
function train!(tm::TMClassifier{ClassType}, x::TMInput, y::ClassType; index::Bool=false, class_weight::Float64=1.0) where ClassType
    ta1::TATeam = tm.clauses[y]
    feedback!(tm, ta1, x, ta1.positive_clauses, ta1.positive_clauses_inverted, ta1.negative_clauses, ta1.negative_clauses_inverted, ta1.positive_included_literals, ta1.positive_included_literals_inverted, ta1.negative_included_literals, ta1.negative_included_literals_inverted, ta1.positive_included_literals_sum, ta1.positive_included_literals_sum_clamp, ta1.negative_included_literals_sum, ta1.negative_included_literals_sum_clamp, ta1.positive_included_literals_idx, ta1.negative_included_literals_idx, true, index, class_weight=class_weight)
    @inbounds for (cls, ta) in tm.clauses
        if cls !== y
            feedback!(tm, ta, x, ta.negative_clauses, ta.negative_clauses_inverted, ta.positive_clauses, ta.positive_clauses_inverted, ta.negative_included_literals, ta.negative_included_literals_inverted, ta.positive_included_literals, ta.positive_included_literals_inverted, ta.negative_included_literals_sum, ta.negative_included_literals_sum_clamp, ta.positive_included_literals_sum, ta.positive_included_literals_sum_clamp, ta.negative_included_literals_idx, ta.positive_included_literals_idx, false, index, class_weight=class_weight)
        end
    end
end


function train!(tm::TMClassifier{ClassType}, X::Vector{TMInput}, Y::Vector{ClassType}; shuffle::Bool=true, index::Bool=false, class_weights::Union{Dict{ClassType, Float64}, Nothing}=nothing) where ClassType
    @threads for i in ifelse(shuffle, randperm(length(Y)), eachindex(Y))
        w = isnothing(class_weights) ? 1.0 : get(class_weights, Y[i], 1.0)
        train!(tm, X[i], Y[i], index=index, class_weight=w)
    end
end


function train!(tm::TMClassifier{ClassType}, x_train::Vector{TMInput}, y_train::Vector{ClassType}, x_test::Vector{TMInput}, y_test::Vector{ClassType}, epochs::Int64; shuffle::Bool=true, index::Bool=false, verbose::Int=1, best_tms_size::Int64=0, best_tms_compile::Bool=true, class_weights::Union{Dict{ClassType, Float64}, Nothing}=nothing)::Vector{Tuple{TMClassifier, Float64}} where ClassType
    ...
    all_time = @elapsed begin
        @inbounds for i in 1:epochs
            training_time = @elapsed train!(tm, x_train, y_train, shuffle=shuffle, index=index, class_weights=class_weights)
            ...
            if verbose > 0
                # Fix 3 — Macro F1 tracking per epoch
                if verbose >= 2 && ClassType != Bool
                    cls_list = sort(collect(keys(tm.clauses)))
                    macro_f1 = 0.0
                    parts = String[]
                    for c in cls_list
                        tp = sum((predicted .== c) .& (y_test .== c))
                        fp = sum((predicted .== c) .& (y_test .!= c))
                        fn = sum((predicted .!= c) .& (y_test .== c))
                        p = tp + fp == 0 ? 0.0 : tp / (tp + fp)
                        r = tp + fn == 0 ? 0.0 : tp / (tp + fn)
                        f1 = p + r == 0.0 ? 0.0 : 2p * r / (p + r)
                        macro_f1 += f1
                        push!(parts, @sprintf("%s=%.3f", c, f1))
                    end
                    macro_f1 /= length(cls_list)
                    @printf("#%s  Acc: %.2f%%  F1: %.4f  [%s]  Best: %.2f%%  Train: %.3fs  Test: %.3fs\n",
                        i, acc * 100, macro_f1, join(parts, " "), best_acc * 100, training_time, testing_time)
                else
