...
def _write_html(out_path: Path, atlas_doc: dict) -> Path:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.html:
        _write_html(Path(args.html), doc)
        print(f"wrote {args.html}  (open in a browser)", file=sys.stderr)
    ...
    if args.csv:
        ...
        try:
            ...
            print(f"warning: prediction unavailable ({exc!r}); 'predicted' column left empty", file=sys.stderr)
