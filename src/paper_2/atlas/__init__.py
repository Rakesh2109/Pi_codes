...
class FPTMInspector:
    ...
    def get_clauses(self) -> list[Clause]:
        ...
        feat_idx, thresh = adapter.feat_idx, adapter.thresh
            ...
            for pol_key, polarity in ((self._pos_key, "positive"), (self._neg_key, "negative")):
                for cl in spec.get(pol_key) or []:
                    ...
                    for bit in cl.get(self._inc_key) or []:
                        literals.append(self._literal(int(bit), GEQ, feat_idx, thresh, names))
                    for bit in cl.get(self._exc_key) or []:
                        literals.append(self._literal(int(bit), LT, feat_idx, thresh, names))
                    weight = ClauseWeight(
    ...
    def _literal(
        bit: int,
        operator: str,
        feat_idx: NDArray[np.int64],
        thresh: NDArray[np.float32],
        names: Sequence[str],
    ) -> Literal:
        fi = int(feat_idx[bit])
        return Literal(feature=names[fi], operator=operator, threshold=float(thresh[bit]))
