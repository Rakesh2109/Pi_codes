...
Two accuracy changes over v1:
  ...
  (b) Local gap perturbation — each quantile edge is nudged into the
                               highest-density neighbourhood by trying
                               t and t ± (local gap)/2, and picking the
                               placement that maximises bit variance.

(The entropy-based dead-bit elimination stage of earlier drafts was
removed: on the evaluated datasets it never fired — quantile placement
already keeps every bit far from p≈0/1 — so it added cost without effect.)
...
class GLADEv2:
    ...
    def fit(self, X: np.ndarray) -> "GLADEv2":
        ...
        # Numerical columns: hybrid budget + gap-aware placement + local refinement.
        self._num_edges = []
    ...
    def _find_edges(self, col: np.ndarray, k: int) -> np.ndarray:
        ...
        # (b) Local perturbation — per edge, try a small left/right nudge.
        edges_work = self._local_perturb(work, edges_work)
        return np.sort(np.unique(prefix + list(edges_work)))[:k]
