...
\begin{abstract}
Intrusion detection has become a basic requirement in heterogeneous Internet of Things (IoT) deployments, including industrial control systems, Internet of Medical Things (IoMT) platforms, and other cyber-physical infrastructures. Although conventional machine-learning detectors have achieved strong benchmark performance, their memory footprint, arithmetic cost, and latency may limit their use on edge nodes where security decisions must be made under tight resource constraints. In this paper, a lightweight intrusion-detection pipeline is presented as a viable alternative for such deployments. The pipeline combines a gap-aware binariser, named GLADE, with a Fuzzy Pattern Tsetlin Machine (FPTM) classifier whose inference path is implemented through Boolean logic and bitwise operations. GLADE maps mixed numerical traffic features to compact Boolean vectors by pruning redundant thresholds and controlling feature expansion relative to thermometer encoding. The FPTM then evaluates human-readable propositional clauses while allowing partial clause matches, thereby retaining transparent decision logic under noisy and heterogeneous traffic conditions. The proposed pipeline was evaluated on WUSTL-EHMS-2020, NSL-KDD, TON\_IoT, and MedSec-25. Accuracy and macro-F1 scores close to the strongest conventional baselines were obtained while serialized model sizes were reduced by one to three orders of magnitude. A deployment artifact was also defined in both uncompressed pickle and Zstandard level-1 FBZ formats, with byte-exact prediction equivalence after decoding. On a Raspberry~Pi~5, GLADE binarisation was measured to be up to $19\times$ faster than the standard Tsetlin Machine binariser and up to $11\times$ faster than thermometer encoding, while the reported detection accuracy was preserved. These results indicate that compact Boolean learning can complement conventional machine learning when interpretability, online adaptation, and edge deployment constraints are decisive.
\end{abstract}
...
\begin{itemize}
    \item \textbf{GLADE binariser.} A feature binarisation method is proposed in which short and informative Boolean vectors are produced by adaptive threshold allocation, gap-aware placement, entropy-based pruning, and local refinement.
...
Internet of Medical Things (IoMT) traffic, and to cover differences in
...
Across all datasets, fewer binary features are produced by GLADE than
by \texttt{StandardBinarizer}, with representation size typically
reduced by approximately 45\%--65\%. Comparable classification
...
final column reports the method with the most favorable overall trade-off
...
approximately 31.3\,KB (NSL-KDD), 33.3\,KB (TON-IoT), 38.5\,KB
...
curves for TON-IoT, which has the largest payload. To keep the linear
...
\begin{enumerate}
    \item \textbf{Compressed size:} For TON-IoT, zstd-1 already
...
the Boolean feature space was reduced by 45\%--65\% relative to
thermometer-style encoding while competitive accuracy and macro-F1
